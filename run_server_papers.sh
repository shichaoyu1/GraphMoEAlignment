...
PAPER4_BASELINE="${PAPER4_BASELINE:-latent_concat}"
SPD_DIM="${SPD_DIM:-16}"
...
SPD_GRAPH_POLICY="${SPD_GRAPH_POLICY:-cross_budget}"
SPD_LOCAL_CROSS_MASS="${SPD_LOCAL_CROSS_MASS:-0.35}"
...
for paper in $PAPER_CONFIGS; do
  ...
  paper4_baseline="$(paper_cfg "$paper" BASELINE "$PAPER4_BASELINE")"
  ...
  spd_graph_policy="$(paper_cfg "$paper" SPD_GRAPH_POLICY "$SPD_GRAPH_POLICY")"
  ...
  for run_spec in "${run_specs[@]}"; do
    ...
    cmd=(
      ...
      --paper4_baseline "$paper4_baseline"
      --spd_dim "$spd_dim"
      ...
      --spd_graph_policy "$spd_graph_policy"
